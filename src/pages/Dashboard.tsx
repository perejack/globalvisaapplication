...
const Dashboard = () => {
  ...
  useEffect(() => {
    // Check if popup was previously shown for this card
    const dismissedKey = `activation_popup_dismissed_${cards[selectedCard]?.id || selectedCard}`;
    const wasDismissed = localStorage.getItem(dismissedKey) === 'true';
    
    if (cards.length > 0 && !cards[selectedCard]?.isActive && !wasDismissed) {
  ...
  // Restore popup state on mount and clear on activation
  useEffect(() => {
    const wasOpen = localStorage.getItem('activation_popup_open') === 'true';
    if (wasOpen && cards.length > 0 && !cards[selectedCard]?.isActive) {
      setShowActivationPopup(true);
    }
  }, [cards, selectedCard]);
  ...
  const handleActivateCard = () => {
    ...
    localStorage.setItem('activation_popup_open', 'false');
